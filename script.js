...
function renderArchive(){if(!archiveTrack)return;const list=archiveList();archiveTrack.innerHTML=list.length?list.map((f,i)=>`<article class="archive-card" data-id="${esc(f.tmdbId)}"><div class="archive-card-poster">${posterOf(f)?`<img src="${esc(posterOf(f))}" alt="${esc(titleOf(f))}" loading="lazy">`:"<span>LOADING<br>POSTER</span>"}<span>${String(i+1).padStart(2,"0")}</span></div><div class="archive-card-meta"><span>${esc((f.type||"film").toUpperCase())}</span><span>${esc(yearOf(f))}</span></div><h3>${esc(titleOf(f))}</h3>${archiveActions(f)}</article>`).join(""):`<div class="archive-empty"><span>00</span><p>ARCHIVE IS EMPTY.</p><small>SEARCH FOR A TITLE ABOVE AND KEEP IT FOR LATER.</small></div>`;updateArchiveCounters(list.length);updateCarousel()}bindArchiveCards()function bindArchiveCards(){if(!archiveTrack)return;$(".archive-card",archiveTrack).forEach(card=>card.addEventListener("click",e=>{if(e.target.closest("button,a"))return;openFilmCard(Number(card.dataset.id))}))}
...
function openFilmCard(id){
  ...
  if(!film)return;
  let modal=$("#afterDarkFilmCard");
  if(!modal){
    modal=document.createElement("div");
    modal.id="afterDarkFilmCard";
    modal.className="ad-film-card-modal";
    modal.innerHTML='<div class="ad-film-card-backdrop"></div><div class="ad-film-card-window"><button class="ad-film-card-close" type="button">CLOSE ×</button><div class="ad-film-card-poster"><img alt=""><span class="ad-film-card-file"></span></div><div class="ad-film-card-info"><div class="ad-film-card-kicker">ARCHIVE FILE</div><div class="ad-film-card-number"></div><div class="ad-film-card-meta"></div><h2></h2><p class="ad-film-card-original"></p><p class="ad-film-card-overview">LOADING DESCRIPTION…</p><div class="ad-film-card-extra"></div></div></div>';
    document.body.appendChild(modal);
    const close=()=>{modal.classList.remove("active");document.body.style.overflow=""};
    $(".ad-film-card-close",modal).addEventListener("click",close);
    $(".ad-film-card-backdrop",modal).addEventListener("click",close);
  }
  const list=archiveList(),index=Math.max(0,list.findIndex(x=>Number(x.tmdbId)===Number(id)))+1,n=String(index).padStart(3,"0"),name=titleOf(film);
  $(".ad-film-card-poster img",modal).src=posterOf(film)||"";
  $(".ad-film-card-poster img",modal).alt=name;
  $(".ad-film-card-file",modal).textContent="FILE / "+n;
  $(".ad-film-card-number",modal).textContent=n;
  $(".ad-film-card-meta",modal).textContent=(film.type||"film").toUpperCase()+" · "+yearOf(film)+" · ★ "+Number(film.vote_average||0).toFixed(1);
  $("h2",modal).textContent=name.toUpperCase();
  $(".ad-film-card-original",modal).textContent="";
  $(".ad-film-card-overview",modal).textContent="LOADING DESCRIPTION…";
  $(".ad-film-card-extra",modal).innerHTML="";
  modal.classList.add("active");document.body.style.overflow="hidden";
  const endpoint=film.type==="series"?"/tv/"+id:"/movie/"+id,creditsEndpoint=film.type==="series"?"/tv/"+id+"/credits":"/movie/"+id+"/credits";
  Promise.all([tmdb(endpoint,{language:"ru-RU"}),tmdb(creditsEndpoint,{language:"ru-RU"})]).then(([details,credits])=>{
    $(".ad-film-card-original",modal).textContent=details.original_title||details.original_name||"";
    $(".ad-film-card-overview",modal).textContent=details.overview||"DESCRIPTION IS NOT AVAILABLE.";
    ...
    $(".ad-film-card-extra",modal).innerHTML=[people.length?"<span>"+esc(film.type==="series"?"CREATED BY":"DIRECTED BY")+" · "+esc(people.join(" / "))+"</span>":"",genres.length?"<span>"+esc(genres.join(" / "))+"</span>":""].join("");
  }).catch(()=>{$(".ad-film-card-overview",modal).textContent="DESCRIPTION COULD NOT BE LOADED."});
}
function initArchive(){renderArchive();$('.filter').forEach(b=>b.addEventListener("click",()=>{$('.filter').forEach(x=>x.classList.remove("active"));b.classList.add("active");currentType=b.dataset.type||"all";renderArchive()}));let suppressCardClick=false;archiveTrack?.addEventListener("click",e=>{const target=e.target instanceof Element?e.target:null;const b=target?.closest("[data-action]");if(b){const id=Number(b.dataset.id),f=films.find(x=>Number(x.tmdbId)===id);if(!f)return;if(b.dataset.action==="remove"){films=films.filter(x=>Number(x.tmdbId)!==id);save(ARCHIVE_KEY,films);renderArchive()}else if(b.dataset.action==="watch"){f.watched=true;save(ARCHIVE_KEY,films);renderArchive();window.open(tmdbUrl(f),"_blank","noopener")}else if(b.dataset.action==="rate"){document.querySelector("#ratings")?.scrollIntoView({behavior:"smooth"});openAuthIfNeeded()}return}if(suppressCardClick||target?.closest("button,a"))return;},true);let drag=false,moved=false,start=0,scroll=0;archiveTrack?.addEventListener("pointerdown",e=>{if(e.target.closest("button,a"))return;drag=true;moved=false;start=e.clientX;scroll=archiveTrack.scrollLeft});archiveTrack?.addEventListener("pointermove",e=>{if(!drag)return;const dx=e.clientX-start;if(Math.abs(dx)<8)return;if(!moved){moved=true;archiveTrack.classList.add("is-dragging");try{archiveTrack.setPointerCapture(e.pointerId)}catch{}}archiveTrack.scrollLeft=scroll-dx*1.15});["pointerup","pointercancel","lostpointercapture"].forEach(n=>archiveTrack?.addEventListener(n,()=>{if(moved){suppressCardClick=true;setTimeout(()=>suppressCardClick=false,0)}drag=false;moved=false;archiveTrack.classList.remove("is-dragging")}))}
function initCarousel(){if(!archiveTrack)return;let ui=$(".archive-carousel-ui");if(!ui){ui=document.createElement("div");ui.className="archive-carousel-ui";ui.innerHTML='<div class="archive-carousel-buttons"><button class="archive-carousel-button" data-carousel="prev" type="button">←</button><button class="archive-carousel-button" data-carousel="next" type="button">→</button></div><div class="archive-carousel-progress"></div><div class="archive-carousel-position">01 / 01</div>';$(".archive-track-wrap")?.appendChild(ui)}ui.addEventListener("click",e=>{const b=e.target.closest("[data-carousel]");if(!b)return;const amount=Math.max(280,archiveTrack.clientWidth*.62);archiveTrack.scrollBy({left:(b.dataset.carousel==="next"?amount:-amount),behavior:"smooth"});setTimeout(updateCarousel,450)});archiveTrack.addEventListener("scroll",()=>requestAnimationFrame(updateCarousel),{passive:true})}
