...
async function getProfile(session){if(!session)return null;const{data,error}=await supabaseClient.from("profiles").select("nickname").eq("id",session.user.id).maybeSingle();if(error)throw error;return data}
async function updateAuthButton(session){const b=$(".header-auth-button");if(!b)return;if(!session){b.textContent="LOG IN / REGISTER";localStorage.removeItem(USER_KEY);return}try{const profile=await getProfile(session);const name=profile?.nickname||session.user.user_metadata?.nickname||session.user.email?.split("@")[0]||"CINEMA";b.textContent=name.toUpperCase();localStorage.setItem(USER_KEY,JSON.stringify({name,mode:"supabase"}))}catch(e){const name=session.user.user_metadata?.nickname||session.user.email?.split("@")[0]||"CINEMA";b.textContent=name.toUpperCase();localStorage.setItem(USER_KEY,JSON.stringify({name,mode:"supabase"}))}}
