...
window.removeArchiveFilmLocal=function(id){films=films.filter(x=>Number(x.tmdbId)!==Number(id));save(ARCHIVE_KEY,films);renderArchive()};
...
function renderSearch(items){window.__searchResults=items;if(!searchResults)return;searchResults.innerHTML=items.length?`<div class="search-results-head"><span>SEARCH RESULTS</span><span>${String(items.length).padStart(2,"0")} TITLES</span></div>`+items.map((f,i)=>{const inA=films.some(x=>Number(x.tmdbId)===Number(f.id));const original=f.original_title||f.original_name||"";const overview=(f.overview||"").trim();const desc=overview.length>150?overview.slice(0,150).replace(/\s+\S*$/,"")+"…":overview;const country=(f.origin_country||[]).slice(0,2).join(" / ");return `<article class="search-result" data-id="${esc(f.id)}"><span class="search-result-index">${String(i+1).padStart(2,"0")}</span><div class="search-result-poster">${f.poster_path?`<img src="${esc(posterOf(f))}" alt="${esc(titleOf(f))}" loading="lazy">`:`<span>NO<br>IMAGE</span>`}</div><div class="search-result-info"><div class="search-result-topline"><span>${esc(typeOf(f).toUpperCase())}</span><span>${esc(yearOf(f))}</span>${country?`<span>${esc(country)}</span>`:""}</div><h3>${esc(titleOf(f))}</h3>${original&&original!==titleOf(f)?`<p class="search-result-original">${esc(original)}</p>`:""}<p class="search-result-overview">${esc(desc||"DESCRIPTION NOT AVAILABLE")}</p><div class="search-result-rating">★ ${Number(f.vote_average||0).toFixed(1)} <span>/ 10</span></div></div><button class="search-result-add ${inA?"added":""}" type="button" data-action="add">${inA?"IN ARCHIVE":"ADD +"}</button></article>`}).join(""):""}async function search(){const q=searchInput?.value.trim();if(!q){searchResults.innerHTML="";return}searchController?.abort();searchController=new AbortController();searchResults.innerHTML='<div class="search-state">SEARCHING DATABASE<span></span></div>';try{const d=await tmdb("/search/multi",{query:q,include_adult:"false",language:"ru-RU",page:1},searchController.signal);renderSearch((d.results||[]).filter(x=>x.media_type==="movie"||x.media_type==="tv").slice(0,12))}catch(e){if(e.name!=="AbortError")searchResults.innerHTML='<div class="search-state">DATABASE CONNECTION FAILED</div>'}}
...
function buildDiscoverParams(){const p={language:"ru-RU",page:1,sort_by:"popularity.desc",include_adult:"false"};const y=movieNight?.year||"any";const exact=/^(\d{4})-\1$/.test(y);const year=exact?Number(y.slice(0,4)):0;const currentYear=new Date().getFullYear();p["vote_count.gte"]=exact&&year>=currentYear-1?0:20;if(movieNight.genre!=="any")p.with_genres=GENRES[movieNight.genre];if(movieNight.country!=="any")p.with_origin_country=movieNight.country;if(movieNight.rating)p["vote_average.gte"]=movieNight.rating;return p}
...
(function(){const f=document.createElement('script');f.src='after-dark-final.js?v=20260920-final2';f.async=false;document.body.appendChild(f)})();
