...
function initHeaderAuth(){const meta=$(".header-meta"),old=$("#authButton");if(old)old.style.display="none";if(!meta||$(".header-auth-button"))return;const b=document.createElement("button");b.className="header-auth-button";b.type="button";b.textContent="LOG IN / REGISTER";b.addEventListener("click",openAuth);const index=$(".index-trigger");meta.insertBefore(b,index||null)}
...
function initArchive(){renderArchive();$$('.filter').forEach(b=>b.addEventListener("click",()=>{$$('.filter').forEach(x=>x.classList.remove("active"));b.classList.add("active");currentType=b.dataset.type||"all";renderArchive()}));archiveTrack?.addEventListener("click",e=>{const b=e.target.closest("[data-action]");if(!b)return;const id=Number(b.dataset.id),f=films.find(x=>Number(x.tmdbId)===id);if(!f)return;if(b.dataset.action==="remove"){films=films.filter(x=>Number(x.tmdbId)!==id);save(ARCHIVE_KEY,films);renderArchive()}else if(b.dataset.action==="watch"){f.watched=true;save(ARCHIVE_KEY,films);renderArchive();window.open(tmdbUrl(f),"_blank","noopener")}else if(b.dataset.action==="rate"){document.querySelector("#ratings")?.scrollIntoView({behavior:"smooth"});openAuthIfNeeded()}});let drag=false,start=0,scroll=0;archiveTrack?.addEventListener("pointerdown",e=>{if(e.target.closest("button,a"))return;drag=true;start=e.clientX;scroll=archiveTrack.scrollLeft;archiveTrack.setPointerCapture(e.pointerId);archiveTrack.classList.add("is-dragging")});archiveTrack?.addEventListener("pointermove",e=>{if(drag)archiveTrack.scrollLeft=scroll-(e.clientX-start)*1.15});["pointerup","pointercancel","lostpointercapture"].forEach(n=>archiveTrack?.addEventListener(n,()=>{drag=false;archiveTrack.classList.remove("is-dragging")}))}
...
function renderRatings(){const list=$("#ratingsList");if(!list)return;let data=ratings.filter(r=>ratingFilter==="all"||r.type===ratingFilter);data.sort((a,b)=>{const av=(Number(a.polina||0)+Number(a.nastya||0))/2,bv=(Number(b.polina||0)+Number(b.nastya||0))/2;if(ratingSort==="low")return av-bv;if(ratingSort==="recent")return new Date(b.date)-new Date(a.date);if(ratingSort==="oldest")return new Date(a.date)-new Date(b.date);return bv-av});list.innerHTML=data.length?data.map((r,i)=>{const avg=((Number(r.polina||0)+Number(r.nastya||0))/2).toFixed(1);return`<article class="rating-card"><span class="rating-number">${String(i+1).padStart(2,"0")}</span><div><h3 class="rating-title">${esc(r.title)}</h3><p class="rating-meta">${esc((r.type||"film").toUpperCase())} · ${esc(r.year)} · ${esc(r.date||"")}</p></div><div class="rating-scores"><div class="rating-score">POLINA<strong>${Number(r.polina||0).toFixed(1)}</strong></div><div class="rating-score">NASTYA<strong>${Number(r.nastya||0).toFixed(1)}</strong></div></div><div class="rating-average">AVG<strong>${avg}</strong></div></article>`}).join(""):`<div class="rating-empty">NO RATINGS YET.</div>`}
function initRatings(){$$(".rating-filter").forEach(b=>b.addEventListener("click",()=>{ratingFilter=b.dataset.ratingFilter||"all";$$('.rating-filter').forEach(x=>x.classList.remove("active"));b.classList.add("active");renderRatings()}));$$('.sort-button').forEach(b=>b.addEventListener("click",()=>{ratingSort=b.dataset.sort||"high";$$('.sort-button').forEach(x=>x.classList.remove("active"));b.classList.add("active");renderRatings()}));renderRatings()}
let authMode="login";
function openAuth(){const m=$("#authModal");if(!m)return;m.classList.add("active");document.body.style.overflow="hidden";syncAuthForm()}
function openAuthIfNeeded(){if(!localStorage.getItem(USER_KEY))openAuth()}
function closeAuth(){$("#authModal")?.classList.remove("active");document.body.style.overflow=""}
function syncAuthForm(){const name=$("#authName"),submit=$("#authForm button"),tabs=$$(".auth-tab");if(name){name.style.display=authMode==="register"?"block":"none";name.required=authMode==="register"}if(submit)submit.textContent=authMode==="register"?"CREATE ACCOUNT ↗":"ENTER ↗";tabs.forEach(t=>t.classList.toggle("active",t.dataset.auth===authMode))}
function initAuth(){$("#authClose")?.addEventListener("click",closeAuth);$("#authModal")?.addEventListener("click",e=>{if(e.target.id==="authModal")closeAuth()});$$('.auth-tab').forEach(t=>t.addEventListener("click",()=>{authMode=t.dataset.auth||"login";syncAuthForm()}));$("#authForm")?.addEventListener("submit",e=>{e.preventDefault();const name=$("#authName")?.value.trim()||"Cinema";localStorage.setItem(USER_KEY,JSON.stringify({name,mode:authMode}));closeAuth();const b=$(".header-auth-button");if(b)b.textContent=name.toUpperCase()})}
