...
function openFilmDetail(id){
  ...
  if(!modal){
    ...
    modal.innerHTML=\`<div class="ad-film-detail-backdrop"></div><div class="ad-film-detail-window" role="dialog" aria-modal="true"><button class="ad-film-detail-close" type="button">CLOSE ×</button><div class="ad-film-detail-grid"><div class="ad-film-detail-poster"><img class="ad-film-detail-image" alt=""><span class="ad-film-detail-file"></span></div><div class="ad-film-detail-content"><div class="ad-film-detail-kicker">ARCHIVE FILE</div><div class="ad-film-detail-number"></div><div class="ad-film-detail-meta"></div><h2 class="ad-film-detail-title"></h2><p class="ad-film-detail-original"></p><p class="ad-film-detail-overview"></p><div class="ad-film-detail-extra"></div></div></div></div>\`;
  ...
  const list=archiveList(),index=list.findIndex(x=>Number(x.tmdbId)===Number(id))+1;
  const image=$(".ad-film-detail-image",modal),file=$(".ad-film-detail-file",modal),num=$(".ad-film-detail-number",modal),meta=$(".ad-film-detail-meta",modal),title=$(".ad-film-detail-title",modal),original=$(".ad-film-detail-original",modal),overview=$(".ad-film-detail-overview",modal),extra=$(".ad-film-detail-extra",modal);
  const name=titleOf(film), n=String(Math.max(1,index)).padStart(3,"0");
  image.src=posterOf(film)||""; image.alt=name; file.textContent=\`FILE / \${n}\`; num.textContent=n;
  meta.textContent=\`\${(film.type||"film").toUpperCase()} · \${yearOf(film)} · ★ \${Number(film.vote_average||0).toFixed(1)}\`;
  title.textContent=name.toUpperCase(); original.textContent=""; overview.textContent="LOADING DESCRIPTION…"; extra.textContent="";
  ...
  Promise.all([tmdb(endpoint,{language:"ru-RU"}),tmdb(creditsEndpoint,{language:"ru-RU"})]).then(([details,credits])=>{
    ...
    extra.innerHTML=[people.length?\`<span>\${esc(film.type==="series"?"CREATED BY":"DIRECTED BY")} · \${esc(people.join(" / "))}</span>\`:"",genres.length?\`<span>\${esc(genres.join(" / "))}</span>\`:"",countries.length?\`<span>\${esc(countries.join(" / "))}</span>\`:""].join("");
  }).catch(err=>{console.error("FILM DETAIL",err);overview.textContent="DESCRIPTION COULD NOT BE LOADED."});
}
function initArchive(){renderArchive();$('.filter').forEach(b=>b.addEventListener("click",()=>{$$('.filter').forEach(x=>x.classList.remove("active"));b.classList.add("active");currentType=b.dataset.type||"all";renderArchive()}));archiveTrack?.addEventListener("click",e=>{const b=e.target.closest("[data-action]");if(b){const id=Number(b.dataset.id),f=films.find(x=>Number(x.tmdbId)===id);if(!f)return;if(b.dataset.action==="remove"){films=films.filter(x=>Number(x.tmdbId)!==id);save(ARCHIVE_KEY,films);renderArchive()}else if(b.dataset.action==="watch"){f.watched=true;save(ARCHIVE_KEY,films);renderArchive();window.open(tmdbUrl(f),"_blank","noopener")}else if(b.dataset.action==="rate"){document.querySelector("#ratings")?.scrollIntoView({behavior:"smooth"});openAuthIfNeeded()}return}if(e.target.closest("button,a"))return;const card=e.target.closest(".archive-card");if(card)openFilmDetail(Number(card.dataset.id))});let drag=false,moved=false,start=0,scroll=0;archiveTrack?.addEventListener("pointerdown",e=>{if(e.target.closest("button,a"))return;drag=true;moved=false;start=e.clientX;scroll=archiveTrack.scrollLeft});archiveTrack?.addEventListener("pointermove",e=>{if(!drag)return;const dx=e.clientX-start;if(Math.abs(dx)<8)return;if(!moved){moved=true;archiveTrack.classList.add("is-dragging");try{archiveTrack.setPointerCapture(e.pointerId)}catch{}}archiveTrack.scrollLeft=scroll-dx*1.15});["pointerup","pointercancel","lostpointercapture"].forEach(n=>archiveTrack?.addEventListener(n,()=>{drag=false;moved=false;archiveTrack.classList.remove("is-dragging")}))}
