async function archiveSyncLoad(user){
  if(!user)return null;
  try{
...
const archiveSyncReady=(async()=>{
  try{
    ...
    if(error)throw error;
    if(!session?.user){
      console.warn("ARCHIVE SYNC: NO AUTH SESSION");
      return null;
    }
    return archiveSyncLoad(session.user);
  }catch(e){
...
async function archiveSyncUpsertMany(list,user){
  ...
  const{data,error}=await supabaseClient
    .from("archive")
    .upsert(list.map(f=>archiveSyncRow(f,user)),{onConflict:"tmdb_id"})
    .select();
  if(error){
...
async function archiveSyncUpsert(f){
  ...
  const{data,error}=await supabaseClient
    .from("archive")
    .upsert(archiveSyncRow(f,user),{onConflict:"tmdb_id"})
    .select()
    .single();
  if(error){
    console.error("ARCHIVE SAVE FAILED",error);
...
async function archiveSyncWatch(id,watched){
  ...
  const{error}=await supabaseClient
    .from("archive")
    .update({watched:!!watched,updated_at:new Date().toISOString()})
...
supabaseReady.then(()=>{
  supabaseClient.auth.onAuthStateChange((event,session)=>{
    if(session?.user&&(event==="INITIAL_SESSION"||event==="SIGNED_IN"||event==="TOKEN_REFRESHED")){
      setTimeout(()=>archiveSyncLoad(session.user),0);
...
(()=>{
  ...
  runtime.src="site-runtime-fix.js";
  runtime.async=false;
