(()=>{
  const state={filter:"all",sort:"high",rows:[],session:null,profile:null};
  let modal=null;
  ...
  function getFilm(id){return state.rows.find(x=>Number(x.tmdb_id)===Number(id))?.film||window.__archiveFilms?.find(x=>Number(x.tmdbId)===Number(id));}
  ...
  function render(){
    ...
    list.innerHTML=data.length?data.map((r,i)=>{
      const polina=r.byPolina?.rating;
      const nastya=r.byNastya?.rating;
      ...
      return `<article class="rating-card">
        ...
        <div class="rating-scores">
          <div class="rating-score">POLINA<strong>${polina==null?"—":Number(polina).toFixed(1)}</strong></div>
          <div class="rating-score">NASTYA<strong>${nastya==null?"—":Number(nastya).toFixed(1)}</strong></div>
        </div>
  ...
  async function loadRatings(){
    try{
      ...
      state.rows=[...grouped.values()].map(g=>{
        ...
        return{...g,byPolina,byNastya,byUser,average:values.length?values.reduce((a,b)=>a+b,0)/values.length:null,latest:g.items.reduce((max,x)=>x.updated_at>x.updated_at?max:x.updated_at,"")};
      });
      render();
    }catch(e){
  ...
  async function init(){
    addStyles();
    intercept();
    try{await supabaseReady;await loadRatings()}catch(e){console.error("SUPABASE RATINGS INIT FAILED",e)}
  }
