(()=>{
  ...
  function initMovieNightFixed(){
    ...
    document.querySelector("#movieNightAdd")?.addEventListener("click",async()=>{if(!movieNight.result)return;const id=Number(movieNight.result.id);let savedFilm=films.find(x=>Number(x.tmdbId)===id);if(!savedFilm){addFilm(movieNight.result);savedFilm=films.find(x=>Number(x.tmdbId)===id)}if(!savedFilm){updateMovieNightButtonsFixed();return}const ok=await archiveSyncUpsert(savedFilm);if(!ok){films=films.filter(x=>Number(x.tmdbId)!==id);save(ARCHIVE_KEY,films);renderArchive();alert("COULD NOT SAVE TO ARCHIVE");return}updateMovieNightButtonsFixed()});
