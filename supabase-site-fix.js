(()=>{
  ...
  function installAuthFallback(){
    ...
    form.addEventListener("submit",async e=>{
      ...
      try{
        await supabaseReady;
        if(mode==="register"){
          const{data,error}=await supabaseClient.auth.signUp({email,password,options:{data:{nickname:name}}});
          if(error)throw error;
          if(data.session){
            closeAuth();
  ...
  async function renderSupabaseRatings(){
    ...
    try{
      session=await getSession();
      if(!session){list.innerHTML='<div class="rating-empty">LOG IN TO VIEW RATINGS.</div>';return;}
      ...
      const filmMap=new Map((filmsData||[]).map(f=>[Number(f.tmdb_id),f]));
      const grouped=new Map();

      (ratingData||[]).forEach(row=>{
        const film=filmMap.get(Number(row.tmdb_id));
        if(!film)return;
        if(!grouped.has(Number(row.tmdb_id)))grouped.set(Number(row.tmdb_id),{film,items:[]});
        grouped.get(Number(row.tmdb_id)).items.push(row);
      });

      let rows=[...grouped.values()].map(group=>{
        const values=group.items.map(x=>Number(x.rating)).filter(Number.isFinite);
        const latest=group.items.reduce((max,x)=>x.updated_at>max?x.updated_at:max,"");
        return{...group,average:values.length?values.reduce((a,b)=>a+b,0)/values.length:null,latest};
      });

      rows=rows.filter(r=>filterMode==="all"||r.film.type===filterMode);
      rows.sort((a,b)=>{
        if(sortMode==="recent")return new Date(b.latest||0)-new Date(a.latest||0);
        if(sortMode==="oldest")return new Date(a.latest||0)-new Date(b.latest||0);
        return sortMode==="low"?(a.average??-1)-(b.average??-1):(b.average??-1)-(a.average??-1);
      });
      ...
      list.innerHTML=rows.length?rows.map((row,index)=>{
        ...
        const peopleHtml=people.map(person=>{
          const value=byUser.get(person.id);
          return `<div class="rating-score">${esc(String(person.nickname).toUpperCase())}<strong>${value==null?"—":Number(value).toFixed(1)}</strong></div>`;
        }).join("");
        const avg=row.average==null?"—":row.average.toFixed(1);
        const mine=row.items.some(item=>item.user_id===session.user.id);
      ...
      console.error("SUPABASE RATINGS RENDER FAILED",error);
      list.innerHTML='<div class="rating-empty">RATINGS CONNECTION FAILED.</div>';
    }
  }

  function bindRatingControls(){
    document.querySelectorAll("#ratings .rating-filter").forEach(button=>button.addEventListener("click",()=>{
      filterMode=button.dataset.ratingFilter||"all";
      document.querySelectorAll("#ratings .rating-filter").forEach(x=>x.classList.remove("active"));
      button.classList.add("active");
      renderSupabaseRatings();
    },true));
    document.querySelectorAll("#ratings .sort-button").forEach(button=>button.addEventListener("click",()=>{
      sortMode=button.dataset.sort||"high";
      document.querySelectorAll("#ratings .sort-button").forEach(x=>x.classList.remove("active"));
      button.classList.add("active");
      renderSupabaseRatings();
    },true));
  }
  ...
  async function init(){
    try{
      ...
      console.error("SITE SUPABASE FIX FAILED",error);
    }
  }
